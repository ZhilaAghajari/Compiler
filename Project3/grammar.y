...
MethodDecl	:	METHODnum Type IDnum LPARENnum Formal_Parameter_List RPARENnum Block
		...
		;
/* This currently creates a tree that looks tree given in the assignment pdf */
/* The type tree is saved here but it could be saved as a semantic action in 
   Grammer rules that match the type like Type { type_tree = $1 } */
Type		:	IDnum Type_rec
			{
...
Formal_Parameter_List_rec:	Formal_Parameter
		...
		;	
Formal_Parameter:	VALnum {type_record = 1;} INTnum Formal_Parameter_rec
			{
		...
		|	INTnum {type_record = 0;}Formal_Parameter_rec
			{
				//$$ = $2;
				$$ = $3;
			}
...
Formal_Parameter_rec:	IDnum 	
			{
				...
				if (type_record == 1) {
					$$ = MakeTree(VArgTypeOp, idTree, NullExp());
				} else {
